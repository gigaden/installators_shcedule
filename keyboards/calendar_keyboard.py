...
def create_calendar_keyboard(year: int, month: int) -> InlineKeyboardMarkup:
    ...
    day_abbr = [
        InlineKeyboardButton(text=i, callback_data=i) for i in calendar.day_abbr
    ]
    make_report: InlineKeyboardButton = InlineKeyboardButton(
        text=LEXICON_CALENDAR["make_report_btn"],
        callback_data=CallBackMakeReport(year=year, month=month).pack(),
    )
    kb_builder.row(
        InlineKeyboardButton(
            text="<<<", callback_data=CallBackMonthBack(year=year, month=month).pack()
        ),
        InlineKeyboardButton(
            text=calendar.month_abbr[month] + " " + str(year), callback_data=month
        ),
        InlineKeyboardButton(
            text=">>>",
            callback_data=CallBackMonthForward(year=year, month=month).pack(),
        ),
    )
    statistic: InlineKeyboardButton = InlineKeyboardButton(
        text=LEXICON_CALENDAR["statistic"],
        callback_data=CallBackStatistic(year=year, month=month).pack(),
    )
    kb_builder.row(*day_abbr, width=7)
    kb_builder.row(*buttons, width=7)
    kb_builder.row(make_report, width=1)
    kb_builder.row(statistic, width=1)
...
def create_edit_day_keyboard(year: int, month: int, day: int) -> InlineKeyboardMarkup:
    kb_builder: InlineKeyboardBuilder = InlineKeyboardBuilder()
    address: InlineKeyboardButton = InlineKeyboardButton(
        text=LEXICON_CALENDAR["add_address_btn"],
        callback_data=CallBackAddress(year=year, month=month, day=day).pack(),
    )
    close: InlineKeyboardButton = InlineKeyboardButton(
        text=LEXICON_CALENDAR["close_address_btn"],
        callback_data=CallBackCloseDay().pack(),
    )
    addresses: InlineKeyboardButton = InlineKeyboardButton(
...
def create_edit_selected_day_keyboard(
    ...
    update_button: InlineKeyboardButton = InlineKeyboardButton(
        text=LEXICON_CALENDAR["update"],
        callback_data=CallBackUpdateAddress(
            tg_id=tg_id, id_address=id_address, year=year, month=month, day=day
        ).pack(),
    )
    del_button: InlineKeyboardButton = InlineKeyboardButton(
        text=f"{LEXICON_CALENDAR['del']}",
        callback_data=CallBackDelAddress(
            tg_id=tg_id, id_address=id_address, year=year, month=month, day=day
        ).pack(),
    )
...
def create_addresses_day_keyboard(
    ...
    addresses_button = [
        InlineKeyboardButton(
            text=f"{address[0][-50:]}",
            callback_data=CallBackEditDay(
                tg_id=tg_id, year=year, month=month, day=day, id_address=address[1]
            ).pack(),
        )
